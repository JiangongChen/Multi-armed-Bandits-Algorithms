...
K = 5e1;    % Total number of experiments
T = 1e4;    % Total rounds of play in each experiment

% Mean of two feedback arms
theta = [0.3,0.2];
gamma = [0.6,0.4];
miu = theta.*gamma;   % Mean of Bernoulli arms, you can choose any number of arms.
...
eta_hedge_dynamic = 0;
eta_hedge_2fed_dynamic = 0;
eta_exp3_dynamic = 1;
eta_exp3_2fed_dynamic = 1;

% Fixed epsilon used in Hedge algorithm
% Notice that if the value is very high, then the perfomance will
% approximate to argmax
eta_hedge = 100;    
eta_hedge_2fed = 100;
...
for i=1:length(flags)
    ...
    elseif flag == 3
        ...
        label = [label "UCB"];
    elseif flag == 4
        all_regrets = Exp3_2fed(K,T,eta_exp3_2fed,theta,gamma,eta_exp3_2fed_dynamic);
        label = [label "Exp3 2-level feedback"];
    elseif flag == 5
        all_regrets = Argmax_algo(K,T,miu);
        label = [label "Argmax"];
    elseif flag == 6
        all_regrets = Hedge_2fed(K,T,eta_hedge_2fed,theta,gamma,eta_hedge_2fed_dynamic);
        label = [label "Hedge 2-level feedback"];
    elseif flag == 7
        all_regrets = TS(K,T,miu);
        label = [label "Thompson Sampling"];
    elseif flag == 8
        all_regrets = TS_2fed(K,T,theta,gamma);
        label = [label "Thompson Sampling 2-level feedbak"];
    else
        error('Please change the value of flag to an integer between 1 to 7!');
    end
